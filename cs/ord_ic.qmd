...
Conditional likelihoods can be computed using the non-exported function `get_ll()`, as below. *Note (June 2026): the code below sums log-likelihoods within each individual/case, resulting in a single log-likelihood value for each individual. This summing would not be neeeded if one were computing an information criterion using the conditional log-likelihoods.*
